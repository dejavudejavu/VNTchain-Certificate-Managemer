...
async function generateMerkleTree(certData) {
    let certSchema = invoke.queryCertificateSchema("v1")
    let certDataArray = [];
...
async function createDigitalSignature(stringToSign, signerEmail) {
    ...
    let signedData = ecdsa.signHex(stringToSign, signerEmail);
    return signedData;
}
...
async function generateCertificateProof(paramsToShare, certUUID, studentEmail) {
    let certSchema = invoke.queryCertificateSchema("v1")
    let certificateDBData=invoke.queryCertificateByUUID(certUUID)
